...
4. If missing: tell the user exactly what's missing and give both options — they paste the token/account ID, or they run `! pnpm cf:deploy` themselves (their own terminal may have the vars, or can complete an interactive `wrangler login` this sandbox can't).

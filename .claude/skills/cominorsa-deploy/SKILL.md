...
- Deploy target is **Cloudflare Workers**, not Pages — `scripts/cloudflare-deploy.sh` runs `wrangler deploy --config wrangler.jsonc --name cominorsa-web` from the project root (T11 cutover: the canonical config is now the root `wrangler.jsonc`, a static-site + Worker config; the old `dist/server/wrangler.json` vinext used to generate no longer exists). A stale `.wrangler/deploy/config.json` from an old `wrangler deploy --dry-run`/`vinext build` can still silently pin Wrangler to a retired config even with the real `wrangler.jsonc` present — `rm -rf .wrangler` (gitignored) if a deploy or `wrangler dev` ever looks like it's serving/targeting the wrong thing.
...
- Separately, **Cloudflare Workers Builds** (a Cloudflare-side integration, not this repo's `ci.yml`) is connected to `cominorsa-web` and builds/deploys on its own schedule outside this repo's control — every pushed branch gets a preview URL, and a push to `main` very likely deploys production automatically. Its build/deploy commands live in the Cloudflare dashboard (Settings → Builds), not in any file here — see DEPLOY.md's "Hecho crítico: Cloudflare Workers Builds" section for what to check there.

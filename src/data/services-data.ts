...
//
// The service catalog's single source of truth (T6a). Moved here,
// pure and framework-free, from `app/services-data.ts` — that module
// also imports `getBaseUrl` (-> `next/headers`) for its Next-specific
// `generateServiceMetadata()`, so importing its *value* exports would
// have pulled `next/headers` into the Bun-native static build at
// module-eval time. `app/services-data.ts` now re-exports the type and
// the array from here unchanged, so Next keeps working exactly as
// before, and `src/build/` imports the same array directly — no
// second hand-copied list to drift out of sync (the problem T3's
// `seguridadMinera`-only copy in `src/build/site-data.ts` had).
